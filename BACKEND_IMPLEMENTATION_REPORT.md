...
Groq uses the equivalent safe codes `GROQ_AUTHENTICATION_FAILED`, `GROQ_RATE_LIMITED`, `GROQ_TIMEOUT`, `GROQ_UNAVAILABLE`, and `GROQ_INVALID_RESPONSE`. Rule fallback follows the same operational-only allowlist; missing credentials and HTTP 401/403 are never hidden by fallback. Provider keys, raw CV text, request bodies, responses, and parsed personal data are not logged.
