...
These fields remain review-only inside `parsed_json`. No migration, profile column, certification model/table, profile suggestion type, automatic application, endpoint, or ranking response was added. `ProfileSyncService` continues to consider only its existing phone, experience, education, and skill allowlist, and matching/scoring/recommendation services do not consume nationality, marital status, or birth date. Synthetic DOCX and fake-provider tests cover extraction, schema validation, normalization, storage, and confirm-flow non-persistence without real candidate data or provider calls.
